...
| `passages.py` | Python 3.10+ (stdlib; imports `app/`) | `practice/read-and-complete/passages/*.md`, `vocab/index.csv` | `score`: `b_text:`, `b_adjust:`, `features:` in each passage's front matter (`--print` writes nothing); `check`: exit 1 on a passage without source, licence, a current `b_text` or 6 blanks. Issue #15; `fetch` comes with #17 |
